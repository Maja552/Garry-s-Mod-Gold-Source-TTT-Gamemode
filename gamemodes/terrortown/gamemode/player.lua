...
function GM:PlayerSetModel(ply)
   ...
   if !table.HasValue(ttt_playermodels, ply:GetModel()) then
...
local deathsounds = {
   Sound("cstrike/player/die1.wav"),
   Sound("cstrike/player/die2.wav"),
   Sound("cstrike/player/die3.wav"),
   Sound("cstrike/player/death6.wav")
}
...
local function PlayDeathSound(victim)
   ...
   if victim:IsZombie() then
      sound.Play("gsttt/zombie/zombie_die"..math.random(1,5)..".wav", victim:GetShootPos(), 150, 100)
   else
      sound.Play(table.Random(deathsounds), victim:GetShootPos(), 130, 100)
   end
end
...
local nextenemydown = 0

function GM:DoPlayerDeath(ply, attacker, dmginfo)
   if ply:IsSpec() then return end

   -- Experimental: Fire a last shot if ironsighting and not headshot
   if GetConVar("ttt_dyingshot"):GetBool() and !ply:IsZombie() then
   ...
   if GetRoundState() == ROUND_ACTIVE then
      ...
      if nextenemydown < CurTime() and ply:GetRole() != ROLE_TRAITOR and IsValid(attacker) and attacker:IsPlayer() and attacker:GetRole() == ROLE_TRAITOR then
         nextenemydown = CurTime() + 1
         for k,v in pairs(player.GetAll()) do
            if v:GetRole() == ROLE_TRAITOR then
               v:SendLua('surface.PlaySound("cstrike/radio/enemydown.wav")')
            end
         end
      end
   ...
   if not (!ply:IsZombie() and (ply.was_headshot or dmginfo:IsDamageType(DMG_SLASH)) or
...
function GM:ScalePlayerDamage(ply, hitgroup, dmginfo)
   ...
   if attacker != ply and IsValid(attacker) and attacker:IsPlayer() and IsValid(wep) and wep:GetClass() == "weapon_ttt_golden_deagle" then
      if ply:IsZombie() then
         dmginfo:ScaleDamage(0.5)
      elseif (ply:IsTraitor() and attacker:IsTraitor())
         or (ply:GetRole() != ROLE_TRAITOR and attacker:GetRole() != ROLE_TRAITOR) then
   ...
   if IsValid(wep) and wep.IsShotgun and IsValid(attacker) and attacker:IsPlayer() then
      local dist = math.Round(ply:GetPos():Distance(attacker:GetPos()))
      if dist > 100 then
         local mul = 1 - math.Clamp(dist / 1500, 0, 0.85)
         --Entity(1):PrintMessage(HUD_PRINTCENTER, math.Round(dmginfo:GetDamage() * mul) .. "    " .. mul.. "   dist: " .. dist)
   ...
   if ply:HasEquipmentItem(EQUIP_CLOAK) then
   ...
   ply.nextDamageSound = ply.nextDamageSound or 0

   --sounds
   if ply.nextDamageSound < CurTime() then
      if ply:IsZombie() then
         ply:EmitSound("gsttt/zombie/zombie_pain"..math.random(1,5)..".wav")
      else
         if ply.was_headshot then
            ply:EmitSound("cstrike/player/headshot"..math.random(1,3)..".wav")
         else
            ply:EmitSound("cstrike/player/pl_shot1.wav")
         end
      end
      ply.nextDamageSound = CurTime() + 1
   end
...
local fallsounds = {
   Sound("cstrike/player/bhit_flesh-1.wav"),
   Sound("cstrike/player/bhit_flesh-2.wav"),
   Sound("cstrike/player/bhit_flesh-3.wav")
};
...
function GM:OnPlayerHitGround(ply, in_water, on_floater, speed)
   ...
   if math.floor(damage) > 0 and !ply:HasEquipmentItem(EQUIP_BOOTS_JUMP) then
      local dmg = DamageInfo()
      ...
      if damage > 5 then
         ...
         ply:SendLua('surface.PlaySound("cstrike/player/pl_fallpain1.wav")') 
      end
   end
end
