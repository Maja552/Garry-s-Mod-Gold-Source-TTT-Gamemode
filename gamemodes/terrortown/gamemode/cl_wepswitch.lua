...
local col_active = {
   tip = {
      ...
      [ROLE_ZOMBIE]    = Color(122, 180, 40, 255),
...
local col_dark = {
   tip = {
      ...
      [ROLE_ZOMBIE]    = Color(133, 160, 50, 155),
...
function WSWITCH:DoSelect(idx)
   ...
   EmitSound("gsttt/wpn_moveselect.wav", Vector(0,0,0), -1, CHAN_AUTO, 0.25, 75, 0, 100, 0)
...
function WSWITCH:ConfirmSelection(noHide)
   ...
   for k, w in pairs(self.WeaponCache) do
      if k == self.Selected and IsValid(w) then
         ...
         if w != LocalPlayer():GetActiveWeapon() then
            EmitSound("gsttt/wpn_hudon.wav", Vector(0,0,0), -1, CHAN_AUTO, 0.25, 75, 0, 100, 0)
         end
         --surface.PlaySound("gsttt/wpn_hudon.wav")
         --sound.Play("gsttt/wpn_hudon.wav", Vector(0, 0, 0), 100, 100, 1)
         return
      end
   end
end
