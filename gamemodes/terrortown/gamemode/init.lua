...
resource.AddWorkshop("2887337293")
resource.AddWorkshop("2887345480")
resource.AddWorkshop("2887345440")
...
AddCSLuaFile("kanade/cl_motd.lua")
...
-- Round times
CreateConVar("ttt_roundtime_minutes", "8", FCVAR_NOTIFY)
CreateConVar("ttt_preptime_seconds", "40", FCVAR_NOTIFY)
...
CreateConVar("ttt_haste_starting_minutes", "6", FCVAR_NOTIFY)
...
CreateConVar("ttt_traitor_max", "7")

CreateConVar("ttt_detective_pct", "0.13", FCVAR_NOTIFY)
CreateConVar("ttt_detective_max", "3")
CreateConVar("ttt_detective_min_players", "7")
CreateConVar("ttt_detective_karma_min", "650")
...
-- Tvirus
CreateConVar("ttt_zombie_scale_dmg", "2")
...
CreateConVar("ttt_round_limit", "8", FCVAR_ARCHIVE + FCVAR_NOTIFY + FCVAR_REPLICATED)
...
CreateConVar("ttt_voice_drain_normal", "0.1", FCVAR_NOTIFY)
CreateConVar("ttt_voice_drain_admin", "0.02", FCVAR_NOTIFY)
CreateConVar("ttt_voice_drain_recharge", "0.1", FCVAR_NOTIFY)
...
function GM:TTTCheckForWin()
   ...
   for k,v in ipairs(player.GetAll()) do
      ...
      if !zombies_exceeded_half_players and (traitor_alive or zombie_alive) and innocent_alive then
         return WIN_NONE --early out
      end
   end
   
   if zombies_exceeded_half_players then
      if zombie_alive then
         if !innocent_alive and !traitor_alive then
            return WIN_ZOMBIE
         end
      else
         return WIN_HUMAN
...
local round_start_traitor_sounds = {
   "elim.wav",
}


hook.Add("TTTPrepareRound", "cs16_prepstartroundsound", function(result)
   local str = 'surface.PlaySound("cstrike/radio/'..table.Random(prep_start_sounds)..'")'
   BroadcastLua(str)
end)

hook.Add("TTTBeginRound", "cs16_startroundsound", function(result)
   for k,v in pairs(player.GetAll()) do
      if v:GetRole() == ROLE_TRAITOR then
         v:SendLua('surface.PlaySound("cstrike/radio/'..table.Random(round_start_traitor_sounds)..'")')
      else
         v:SendLua('surface.PlaySound("cstrike/radio/'..table.Random(round_start_sounds)..'")')
      end
   end
end)

hook.Add("TTTEndRound", "cs16_endrounds", function(result)
   local win_ct = "cstrike/radio/ctwin.wav"
   local win_t = "cstrike/radio/terwin.wav"
   local win_z = "gsttt/zombie/win_zombies"..math.random(1.2)..".mp3"
   local win_h = "gsttt/zombie/win_humans"..math.random(1.2)..".wav"


   if result == WIN_INNOCENT or result == WIN_TIMELIMIT then
      BroadcastLua('surface.PlaySound("'..win_t..'")')

   elseif result == WIN_TRAITOR then
      BroadcastLua('surface.PlaySound("'..win_ct..'")')

   elseif result == WIN_ZOMBIE then
      BroadcastLua('surface.PlaySound("'..win_z..'")')

   elseif result == WIN_HUMAN then
      BroadcastLua('surface.PlaySound("'..win_h..'")')

   end
end)


util.AddNetworkString("gsttt_comms_sabotaged")

function GSTTT_SabotageComms()
   GSTTT_COMMS_SABOTAGED_TIL = CurTime() + 30
   net.Start("gsttt_comms_sabotaged")
   net.Broadcast()

   for k,v in pairs(player.GetAll()) do
      if v:IsTraitor() then
         v:PrintMessage(HUD_PRINTCENTER, "The terrorist comms were sabotaged")
         v:SendLua('surface.PlaySound("cstrike/events/tutor_msg.wav")')
      end
   end
end
...
hook.Add("PostCleanupMap", "gsttt_PostCleanupMap", function()
   for k,v in pairs(ents.GetAll()) do
      if table.HasValue(to_wake_ents, v:GetClass()) then
         print(v, v:GetPos())
         timer.Simple(1, function()
            Entity(1):SetPos(v:GetPos() + Vector(0,0,50))
         end)
...
AddCSLuaFile("kanade/cs16.lua")
AddCSLuaFile("kanade/weapon_fix.lua")
include("kanade/cs16.lua")
include("kanade/tvirus.lua")
include("kanade/weapon_fix.lua")