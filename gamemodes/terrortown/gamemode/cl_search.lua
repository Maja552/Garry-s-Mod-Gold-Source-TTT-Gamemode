...
local TypeToMat = {
   ...
   eq_disg="disguise",
   --eq_cloak="cloak",
   role={[ROLE_TRAITOR]="traitor", [ROLE_DETECTIVE]="det", [ROLE_INNOCENT]="inno", [ROLE_ZOMBIE]="zombie"},
   c4="code",
   dmg=DmgToMat,
   wep=WeaponToIcon,
   head="head",
...
function PreprocSearch(raw)
   ...
   for t, d in pairs(raw) do
      ...
      elseif t == "eq_disg" then
         ...
         /*
      elseif t == "eq_cloak" then
         if d then
            search[t].text = T("search_cloak")
            search[t].p = 18
         end
         */
...
local function ReceiveRagdollSearch()
   ...
   search.eq_cloak = util.BitSet(eq, EQUIP_CLOAK)
