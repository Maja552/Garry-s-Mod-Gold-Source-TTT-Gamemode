...
local function SetCloak(ply, cmd, args)
   if not IsValid(ply) or not ply:IsActiveTraitor() then return end

   if ply:HasEquipmentItem(EQUIP_CLOAK) then
      local state = #args == 1 and tobool(args[1])
      if hook.Run("TTTToggleCloak", ply, state) then return end

      ply.lastCloak = ply.lastCloak or 0
      if CurTime() < ply.lastCloak + 2 then
         --LANG.Msg(ply, "cloak_failed")
         return
      end
      ply.lastCloak = CurTime()

      ply:SetNWBool("cloaked", state)
      if state then
         ply:SetRenderMode(RENDERMODE_TRANSALPHA)
         ply:SetColor(Color(255, 255, 255, 25))
         ply:EmitSound("Weapon_CS_Flashbang.Explode")
      else
         ply:SetRenderMode(RENDERMODE_NORMAL)
         ply:SetColor(Color(255, 255, 255, 255))
      end
      LANG.Msg(ply, state and "cloak_turned_on" or "cloak_turned_off")
   end
end
concommand.Add("ttt_set_cloak", SetCloak)
