...
function GM:PlayerSay(ply, text, team_only)
   ...
   if GetRoundState() == ROUND_ACTIVE then
      local ply_is_spec = ply:Team() == TEAM_SPEC

      if !team_only and ply:IsZombie() then
         return ""
      end

      if !ply_is_spec and GSTTT_COMMS_SABOTAGED_TIL > CurTime() then
         if !ply:IsTraitor() then
            text = ""
            ply:PrintMessage(HUD_PRINTTALK, "The comms seem to be broken...")
         end
         team_only = true
      end

      if ply_is_spec and not DetectiveMode() then
      ...
      elseif team_only and not ply_is_spec and ply:IsSpecial() then
...
function GM:PlayerCanHearPlayersVoice(listener, speaker)
   ...
   if speaker:IsZombie() then
      return (GetRoundState() != ROUND_ACTIVE or listener:IsZombie()), false
   end
   ...
   local comms_sabotaged = (GSTTT_COMMS_SABOTAGED_TIL > CurTime())

   if comms_sabotaged then
      --if speaker == player.GetBySteamID64("76561198115774666") then
      --   print(speaker, speaker.traitor_gvoice)
      --end
      ...
      if speaker:IsActiveTraitor() and !speaker.traitor_gvoice then
         ...
         if dist > 1250 then return false, false end
         return true, true
      end
   end
