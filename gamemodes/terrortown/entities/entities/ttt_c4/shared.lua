...
ENT.Type = "anim"
--ENT.Model = Model("models/cs16/w_c4.mdl")
ENT.Model2 = Model("models/cs/w_c4.mdl")
...
function ENT:Initialize()
   ...
   --self:SetNoDraw(true)
   if SERVER then
      self.NewC4Model = ents.Create("prop_physics")
      self.NewC4Model:SetModel(self.Model2)
      self.NewC4Model:Spawn()
      self.NewC4Model:SetPos(self:GetPos())
      self.NewC4Model:SetAngles(self:GetAngles())
      self.NewC4Model:SetParent(self)
      self.NewC4Model:SetCollisionGroup(COLLISION_GROUP_WEAPON)
   end
end
...
function ENT:Think()
   ...
   elseif self:GetArmed() and CurTime() > self.Beep then
      ...
      if SERVER then
         ...
         if !self.gonnablow and (etime - CurTime()) < 10 then
            self.gonnablow = true
            for k,v in pairs(player.GetAll()) do
               if v:IsTraitor() and v:GetPos():Distance(self:GetPos()) < 500 then
                  v:SendLua('surface.PlaySound("cstrike/radio/blow.wav")')
               end
            end
         end
      end
...
if SERVER then
   ...
   function ENT:Disarm(ply)
      ...
      self:EmitSound("cstrike/weapons/c4_disarmed.wav")

      if ply != owner and IsValid(owner) then
         LANG.Msg(owner, "c4_disarm_warn")
      end

      -- "Bomb has been defused" sound effect, plays on the placer and traitors if placer is a traitor
      if ply.nextbombplsound == nil then
         ply.nextbombplsound = 0
      end

      if ply.nextbombplsound < CurTime() then
         if ply:IsTraitor() then
            for k,v in pairs(player.GetAll()) do
               if v:IsTraitor() and v != ply then
                  v:SendLua('surface.PlaySound("cstrike/radio/bombdef.wav")')
               end
            end
         end
         ply.nextbombplsound = CurTime() + 15
      end
      ply:SendLua('surface.PlaySound("cstrike/radio/bombdef.wav")')
   ...
   function ENT:FailedDisarm(ply)
      ...
      self:SetExplodeTime(CurTime() + 0.5)
   end
   ...
   function ENT:Arm(ply, time)
      ...
      -- "Bomb has been planted" sound effect, plays on the placer and traitors
      if ply.nextbombplsound == nil then
         ply.nextbombplsound = 0
      end
      ...
      if ply.nextbombplsound < CurTime() then
         for k,v in pairs(player.GetAll()) do
            if v:IsTraitor() and v != ply then
               v:SendLua('surface.PlaySound("cstrike/radio/bombpl.wav")')
            end
         end
         ply.nextbombplsound = CurTime() + 15
      end
      ply:SendLua('surface.PlaySound("cstrike/radio/bombpl.wav")')
...
if CLIENT then
   ...
   function ENT:GetTimerPos()
      local ent = self
      local att = ent:GetAttachment(ent:LookupAttachment("controlpanel0_ur"))
      if att then
         return att
      else
         local ang = ent:GetAngles()
         ang:RotateAroundAxis(ent:GetUp(), 180)
         local pos = (ent:GetPos() + ent:GetForward() * -3.5 +
                      ent:GetUp() * 6.5 + ent:GetRight() * 2)
...
if SERVER then
   util.AddNetworkString("GSTTT_StartedDisarming")

   net.Receive("GSTTT_StartedDisarming", function(len, pl)
      if len < 5 and pl:Alive() and pl:Team() != TEAM_SPEC then
         local tr = pl:GetEyeTrace()
         if tr.Entity:GetClass() == "ttt_c4" then
            --print(tr.Entity)
            tr.Entity:EmitSound("cstrike/weapons/c4_disarm.wav")
         end
      end
   end)
end
