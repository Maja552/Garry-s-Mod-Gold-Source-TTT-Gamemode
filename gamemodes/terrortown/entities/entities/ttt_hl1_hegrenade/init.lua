...
function ENT:PushPullRadius(pos, pusher)
	local radius = 700

	util.BlastDamage(self, self:GetThrower(), self:GetPos(), radius / 2, 30)
	...
	for i=1, 200 do
		...
		local tr = util.TraceLine({
			start = pos,
			endpos = pos + (AngleRand():Forward() * 700)
		})
		local ent = tr.Entity
		if ent:IsPlayer() then
			if !table.HasValue(unique_hits, ent) then
				ent:TakeDamage(65, self:GetOwner(), self)
				hits = hits + 1

				table.ForceInsert(unique_hits, ent)
			end
		end
...
function ENT:Explode(pTrace)
	...
	self:PushPullRadius(pos, self:GetThrower())
