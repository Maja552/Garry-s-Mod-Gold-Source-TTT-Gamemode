...
function ENT:Use(ply)
   if IsValid(ply) and ply:IsPlayer() and ply:IsActive() then
      ...
      local pplacer = self:GetPlacer()

      local dmg_info = DamageInfo()
      dmg_info:SetDamage(ply:Health())
      dmg_info:SetInflictor(self)
      if IsValid(pplacer) then
         dmg_info:SetAttacker(pplacer)
      else
         dmg_info:SetAttacker(self.Entity)
      end
      ...
      if SERVER then
         sound.Play("weapons/stunstick/spark"..math.random(1,3)..".wav", ppos, 100, 100, 1)
         sound.Play("ambient/energy/spark"..math.random(5,6)..".wav", ppos, 100, 100, 1)
      end
   end
end
