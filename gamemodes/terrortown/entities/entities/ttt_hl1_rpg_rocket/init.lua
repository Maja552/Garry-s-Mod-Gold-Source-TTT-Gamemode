...
function ENT:Initialize()
	...
	self.dmg = self.ActualDamage
	self.radius = self.dmg * 2.5

	self.nextCanExplode = CurTime() + 0.1
end
...
function ENT:Explode(ent)
	--if self.nextCanExplode < CurTime() or self.didHit then return end
...
function ENT:Think()
	...
	if CurTime() - self.m_flIgniteTime < 1 then
		...
		if self:WaterLevel() == 3 then
			...
			if self:GetVelocity():Length() > self.ActualSpeed then
				self:SetLocalVelocity(self:GetVelocity():GetNormalized() * self.ActualSpeed)
			end
		end
	else
		...
		if self:WaterLevel() == 0 && self:GetVelocity():Length() < 500 then
			self:Explode()
		end
	end
