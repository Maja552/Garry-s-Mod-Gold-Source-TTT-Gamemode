...
function ENT:Initialize()
	...
	if self:GetOwner() != NULL then
		self:EmitSound(self.SoundDeploy, 60, 100, 0.1, CHAN_VOICE)
		self:EmitSound(self.SoundCharge, 60, 100, 0.1, CHAN_BODY)
...
function ENT:PowerupThink()
	...
	if CurTime() > self.m_flPowerUp then
		...
		-- play enabled sound
		self:EmitSound(self.SoundActivate, 60, 75, 0.1, CHAN_VOICE)
		self.PowerUpDelay = nil
	end
	self:NextThink(CurTime() + .1)
end
...
function ENT:BeamBreakThink()
	...
	if tr.Entity:IsPlayer() and tr.Entity:GetRole() == ROLE_TRAITOR then bBlowup = false end
