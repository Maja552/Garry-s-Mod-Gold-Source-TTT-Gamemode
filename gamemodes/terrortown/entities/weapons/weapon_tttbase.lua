   ...
   function SWEP:DrawHUD()
      ...
      if client.IsTraitor and client:IsTraitor() then
         surface.SetDrawColor(255 * bright,
                              ...
                              50 * bright,
                              255 * alpha)
      elseif client.IsZombie and client:IsTraitor() then
         surface.SetDrawColor(162 * bright,
                              255 * bright,
                              1 * bright,
                              255 * alpha)
      else
...
function SWEP:ShootBullet( dmg, recoil, numbul, cone )
   ...
   self:CS16_MuzzleFlash(self.MuzzleFlashSize1, self.MuzzleFlashSize2)
...
function SWEP:DoImpactEffect(pTrace, iDamageType)
   ...
   if tr.HitWorld or !(tr.Entity:IsPlayer() or tr.Entity:GetClass() == "prop_ragdoll") then
      util.Decal("hl.impact", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal)

      local data = EffectData()
      data:SetOrigin(tr.HitPos)
      util.Effect("cs16_impact", data) 
      ParticleEffect("cs16_impact", tr.HitPos + tr.HitNormal, Angle())
